...
The deploy IAM user needs the following inline policy in addition to any
managed policies. Replace `us-east-1` and `<account-id>` with your values;
bucket/table names match the defaults from `terraform.tfvars`
(`project_name = "game-servers"`).
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:s3:::game-servers-tf-state"
    },
    {
    ...
    },
    {
      ...
      "Resource": "arn:aws:dynamodb:us-east-1:<account-id>:table/game-servers-tf-locks"
    },
    {
      "Sid": "EventBridgeTags",
      "Effect": "Allow",
...
The `EventBridgeTags` statement is required because the AWS provider tags
EventBridge rules on creation, and those actions are absent from every
relevant managed policy. Without it `terraform apply` will fail.
