...
| [**Private parent + submodule guide**](https://codercoco.github.io/game-server-deploy/guides/submodule/) | Wrapping this repo in a private repo that holds `terraform.tfvars` and tfstate. Includes an interactive scaffolder (`npm run scripts:init-parent`) that generates the wrapper Makefile, tfvars, and `.env`. |
...
├── scripts/                   # Helper scripts (init-parent CLI scaffolder)
